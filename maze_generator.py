...
#boundary check
    ...
    #randomly select and return one of the unvisited neighbours
    #return false if no unvisited neighbour are available
    ...
    def get_neighbors(self, grid_cells):
        neighbors = []
        directions = [(1, 0, 'right'), (0, 1, 'bottom'), (-1, 0, 'left'), (0, -1, 'top')]
        for dx, dy, direction_name in directions:
            new_x, new_y = self.x + dx, self.y + dy
            if 0 <= new_x < cols and 0 <= new_y < rows:
                neighbor = grid_cells[new_x + new_y * cols]
                if not self.walls[direction_name]:
